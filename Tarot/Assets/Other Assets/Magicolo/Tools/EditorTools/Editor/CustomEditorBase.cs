...
namespace Magicolo.EditorTools {
	public class CustomEditorBase : Editor {
		...
		public bool UniqueNameField(INamable namable, INamable[] namables, GUIContent label, GUIStyle style, params GUILayoutOption[] options) {
			...
			string sequenceName = EditorGUILayout.TextField(label, namable.Name, style, options);
				
			if (EditorGUI.EndChangeCheck()) {
				namable.SetUniqueName(sequenceName, namable.Name, namables);
				changed = true;
			}
		...
		public static void Reorderable(SerializedProperty arrayProperty, int index, bool disableOnPlay, Rect dragArea, ReorderCallback reorderCallback = null) {
			...
			string arrayIdentifier = string.Format("{0}{1}{2}{3}{4}", arrayProperty.name, arrayProperty.arraySize, arrayProperty.depth, arrayProperty.propertyPath, arrayProperty.propertyType);
